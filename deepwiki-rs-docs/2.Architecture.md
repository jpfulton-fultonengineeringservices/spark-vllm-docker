...
# spark-vllm-docker — Architecture Overview
...
`spark-vllm-docker` is **not** an inference engine. It is a **deployment-time mutation and orchestration layer** that customizes an *externally installed* vLLM package without forking it. The repository's architecture is grounded in a single, consistently applied design philosophy:
...
`spark-vllm-docker` delivers business value by **enabling rapid, reproducible customization of vLLM without forking upstream**. Its raison d'être is the tension between two forces:
...
`spark-vllm-docker` resolves this by introducing a **mod abstraction**: an isolated, traceable, reversible unit of change. This reduces time-to-production for cutting-edge models on new GPU hardware and multi-node Spark clusters, without incurring the long-term liability of a vLLM fork.

### 2.2 User Roles and Scenarios

```mermaid
graph TD
    spark-vllm-docker["spark-vllm-docker<br/>patch and mod management"]
    ...
    Eng -->|"Apply targeted fixes without forking"| spark-vllm-docker
    KernelEng -->|"Vendored modifiable FA kernels"| spark-vllm-docker
    Operator -->|"Ready-made 3x/4x/8x recipes"| spark-vllm-docker
```
...
graph TB
    ...
    spark-vllm-docker -->|"Source patching AST/text + process launch"| VLLM
    spark-vllm-docker -->|"Compile-time and runtime dependency"| CUDA
    spark-vllm-docker -->|"API integration via autograd"| Torch
    spark-vllm-docker -->|"Package/artifact source"| NGC
    spark-vllm-docker -->|"Model and parser download"| HF
    spark-vllm-docker -->|"Container runtime and multi-node distribution"| DockerSSH
```
...
The central boundary insight is: **`spark-vllm-docker` mutates the environment at deployment time, but owns none of the runtime engine it mutates.**
