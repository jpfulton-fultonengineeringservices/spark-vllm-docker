...
| Attribute | Value |
| --- | --- |
| **Project Name** | `spark-vllm-docker` |
...
`spark-vllm-docker` is a **patch-and-mod management repository** that enables teams to customize an externally installed **vLLM** inference and serving engine without maintaining a fork of the upstream project. Rather than diverging from the open-source codebase, `spark-vllm-docker` treats customization as a set of **self-contained, traceable, and idempotent modifications** ("mods") that are applied to an installed vLLM package at deployment time.
...
In addition to the mod framework, `spark-vllm-docker` ships a **vendored FlashAttention kernel package** (`inkling_sm120_fa4`) written in **CUTLASS CuTe-DSL**, providing paged-KV attention kernels for modern NVIDIA GPU architectures, and a set of **deployment recipes** for multi-node **DGX Spark clusters**.

### 1.3 Business Value

`spark-vllm-docker` delivers measurable business value by:
...
- **Non-invasive mutation model**: vLLM is patched *in place* at deployment time; `spark-vllm-docker` never owns the engine itself.
...
`spark-vllm-docker` serves three primary user roles, each with distinct needs and usage scenarios.
...
> `spark-vllm-docker` **owns** the mod definitions (patch scripts, diff patches, templates, entry scripts), the vendored `inkling_sm120_fa4` FlashAttention kernel package, and cluster deployment recipes. It **does not own** vLLM itself; it modifies an externally installed vLLM at deployment time.

This boundary is the single most important architectural decision in the system: **`spark-vllm-docker` is a mutator and orchestrator, not an inference engine**. All inference computation is ultimately performed by vLLM and its underlying GPU stack.
...
The following are explicitly **out of scope** for `spark-vllm-docker`:
...
```mermaid
graph TD
    subgraph spark-vllm-docker["spark-vllm-docker (System Boundary)"]
...
`spark-vllm-docker` depends on four external systems, each with a distinct interaction model.
...
vLLM is the open-source LLM inference and serving engine that `spark-vllm-docker` targets. The interaction is **asymmetric and invasive**:

- **Source patching**: `spark-vllm-docker` modifies vLLM's installed `site-packages` source. Patched surfaces include FA4 dispatch logic, weight iterators, the model registry, and chat templates.
- **Process launch**: vLLM's server process is launched by mod entry scripts (`run.sh`), not by `spark-vllm-docker` as a long-running service.
- **Ownership boundary**: `spark-vllm-docker` never ships vLLM; it assumes a pre-installed or image-provided vLLM exists at deployment time.
...
```mermaid
graph LR
    spark-vllm-docker["spark-vllm-docker"]
    VLLM["vLLM Engine"]
    CUDA["NVIDIA CUDA / CUTLASS"]
    Torch["PyTorch"]
    NGC["NVIDIA NGC Containers"]

    spark-vllm-docker -->|"patches source in place"| VLLM
    spark-vllm-docker -->|"vendored kernels compile against"| CUDA
    spark-vllm-docker -->|"autograd Functions exposed to"| Torch
    spark-vllm-docker -->|"may source vLLM artifacts from"| NGC
    VLLM -->|"runs on"| CUDA
    VLLM -->|"uses"| Torch
```
...
- **Coupling risk**: Because `spark-vllm-docker` patches vLLM source textually, upstream vLLM refactors can break mods — this is mitigated by AST validation and fallback ordering.
...
The following diagram presents the C4 SystemContext view: `spark-vllm-docker` at the center, its three user roles as actors, and its four external systems.
...
graph TD
    ...
    spark-vllm-docker["spark-vllm-docker - Patch and Mod Management Repository for vLLM"]
    ...
    InferEng -->|"applies model and quantization mods"| spark-vllm-docker
    KernelEng -->|"develops and tunes attention kernels"| spark-vllm-docker
    ClusterOp -->|"runs cluster recipes and launches servers"| spark-vllm-docker

    spark-vllm-docker -->|"patches source in place and launches server"| VLLM
    spark-vllm-docker -->|"compiles vendored kernels against"| CUDA
    spark-vllm-docker -->|"exposes kernels via autograd Functions to"| Torch
    spark-vllm-docker -->|"optionally sources vLLM artifacts from"| NGC
